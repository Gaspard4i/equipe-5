...
export class Bots {
	constructor(count) {
		this.bots = [];
		this.botIdCounter = 0; // Compteur pour générer des IDs uniques
	...
	createBots(count) {
		for (let i = 0; i < count; i++) {
			...
			while (!isValidPosition) {
				...
				bot = new Player(30, x, y, 0, 0, false, `bot_${this.botIdCounter++}`);
				bot.id = `bot_${this.botIdCounter++}`; // Génère un ID unique avec un préfixe
